...
class TextImageGenerator:
    def __init__(self, img_dirpath, img_w, img_h,
                 batch_size, downsample_factor, max_text_len=32):
        self.img_h = img_h
        self.img_w = img_w
    ...
    def build_data(self):
        ...
        for i, img_file in enumerate(self.img_dir):
            ...
            self.imgs[i, :, :] = img
            txt = img_file.split('_')[0]
            self.Y_len[i] = len(txt)
    ...
    def next_batch(self):       ## batch size만큼 가져오기
        while True:
            X_data = np.ones([self.batch_size, self.img_w, self.img_h, 1])     # (bs, 800, 64, 1)
            ...
            for i in range(self.batch_size):
                ...
                label_length[i] = self.Y_len[i]
            

            # dict 형태로 복사
            inputs = {
                'the_input': X_data,  # (bs, 800, 64, 1)
                'the_labels': Y_data,  # (bs, 98)
